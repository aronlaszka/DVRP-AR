...
num_process = parse(Int64, ARGS[3])

ENV["OMP_NUM_THREADS"] = string(num_process)
ENV["OPENBLAS_NUM_THREADS"] = string(num_process)
ENV["MKL_NUM_THREADS"] = string(num_process)

addprocs(num_process)
...
@everywhere begin
function LoadChains(file_path::String)
    # window is seconds (ie 1800)
    # detour minimum in seconds (ie 900)
    ...
    chains = transform(chains, :pickup_time_since_midnight => ByRow(x -> x) => :e)
    chains = transform(chains, :pickup_end_time_since_midnight => ByRow(x -> x) => :ex)
    chains = transform(chains, :dropoff_start_time_since_midnight => ByRow(x -> x) => :lx)
    chains = transform(chains, :dropoff_time_since_midnight => ByRow(x -> x) => :l)
    return chains
end
...
function GetChain(df_chain, chain_id)
    ...
    for row in eachrow(df)
        push!(result, Request(row.chain_order, row.pickup_node_id, row.dropoff_node_id, row.request_arrive_time, row.am_count, row.wc_count, row.e, row.l, row.ex, row.lx, -1, -1, row.search_time, -1, chain_id))
    end
...
file_path = data_dir * "/matrix/time_matrix.csv"
TRAVEL_TIME_MATRIX = DataFrame(CSV.File(file_path, header=0))
TRAVEL_TIME_MATRIX = Matrix(TRAVEL_TIME_MATRIX)
TRAVEL_TIME_MATRIX = map(ceil, TRAVEL_TIME_MATRIX)
TRAVEL_TIME_MATRIX = map(Int64, TRAVEL_TIME_MATRIX)

node_map_file_name = data_dir * "/matrix/node_map.csv"
node_map_df = DataFrame(CSV.File(node_map_file_name, header=1))
node_map = Dict()
node_map_reverse = Dict()
for row in eachrow(node_map_df)
    node_map[row.osm_id] = row.ttm_id
    node_map_reverse[row.ttm_id] = row.osm_id
end


println("[INFO] Loaded time matrix")

# load chains
DF_TRAIN = LoadChains( data_dir * "/processed/train.csv")
DF_TEST = LoadChains( data_dir * "/processed/test.csv")
end

println("[INFO] Loaded requests")
flush(stdout)
...
struct theta
    ...
    am::Int64 # ambulatory capacity
    wc::Int64 # wheel chair capacity
    node::Int64 # node id
    request_id::Int64
    chain_id::Int64
end
...
function CheckCapacity(m::CentMDP, route_plan::Vector{theta})
    ...
    for loc in route_plan
        if loc.p == 0
            num_am_passengers = num_am_passengers + loc.am
            num_wc_passengers = num_wc_passengers + loc.wc
        else
            num_am_passengers = num_am_passengers - loc.am
            num_wc_passengers = num_wc_passengers - loc.wc
        end
        if num_am_passengers > m.am_capacity
            return false
        end
        if num_wc_passengers > m.wc_capacity
            return false
        end
    end
    return true
...
function CheckFeasibleRoutePlan(s::CentMDPState, m::CentMDP, new_route_plan::Vector{theta}, vehicle_id::Int64)
    ...
    for k in 1:length(route_plan)
        ...
        route_plan[k] = theta(time_to_location, route_plan[k].e, route_plan[k].l, route_plan[k].p, route_plan[k].am, route_plan[k].wc, route_plan[k].node, route_plan[k].request_id, route_plan[k].chain_id)
...
function TSPInsertion(s::CentMDPState, m::CentMDP, current_route_plan::Vector{theta}, vehicle_id::Int64, request::Request)
    ...
    pickup_theta = theta(-1, request.e, request.ex, 0, request.am_capacity, request.wc_capacity, request.pickup_node_id, request.request_id, request.chain_id)
    dropoff_theta = theta(-1, request.lx, request.l, 1, request.am_capacity, request.wc_capacity, request.dropoff_node_id, request.request_id, request.chain_id)
...
function RoutePlanCostBudgetVoid(m::CentMDP, s::CentMDPState, route_plan::Vector{theta}, vehicle_id::Int64)
    result = (m.am_capacity + m.wc_capacity) * (s.requests[length(s.requests)].l - s.time)
    num_wc_passengers = 0
    num_am_passengers = 0
    for loc in route_plan
        if loc.p == 0
    ...
    current_time = s.time
    for loc in route_plan
        result = result - ((num_am_passengers + num_wc_passengers) * (loc.a - current_time))
        if loc.p == 0
            num_am_passengers = num_am_passengers + loc.am
            num_wc_passengers = num_wc_passengers + loc.wc
        else
...
function RoutePlanCostBudget(m::CentMDP, s::CentMDPState, route_plan::Vector{theta}, vehicle_id::Int64)
    ...
    current_time = s.time
    for loc in route_plan
        if num_am_passengers + num_wc_passengers > 0
...
function chain_merge(s::CentMDPState, chain_id::Int64)
    ...
    if length(right_chain) >= 1
        ...
        for i in 1:length(merged_chain)
            req = merged_chain[i]
            push!(result, Request(i, req.pickup_node_id, req.dropoff_node_id, req.request_arrive_time, req.am_capacity, req.wc_capacity, req.e, req.l, req.ex, req.lx, req.actual_pickup_time, req.actual_dropoff_time, req.max_search_time, req.vehicle_id, req.chain_id))
        end
...
function MCTS.build_tree(planner::AbstractMCTSPlanner, s)
    ...
    if request_id <= length(s.requests)
        current_request = s.requests[s.request_id]
        max_search_time = min(planner.solver.max_time, current_request.max_search_time)
        for n = 1:n_iterations
            MCTS.simulate(planner, root, depth)
            if time() - start_us >= max_search_time
                break
            end
        end
    end
...
function GetInitialState(num_vehicles::Int64, test_chain_id::Int64, depot_id::Int64, test::Bool)
    ...
    for i in 1:num_vehicles
        push!(vehicle_locations, theta(time, -1, 172800, 1, 0, 0, depot_id, 0, test_chain_id))
        push!(route_plans, [])
    end
    if test
        requests = requests[1:60]
    end
...
function RunSimulation(num_vehicles::Int64,
    ...
    mdp = CentMDP(num_vehicles, am_capacity, wc_capacity, k_max, vv, budget, depot_id)
...
function FullSim(num_vehicles::Int64,
    ...
    if parallel
        results = pmap(test_chain_id->RunSimulation(num_vehicles, am_capacity, wc_capacity, k_max, vv, budget, depot_id, policy_type, test_chain_id, mcts_iterations, mcts_depth, mcts_num_chains, mcts_runtime, test), [x for x in 1:length(test_chain_ids)])
    else
        for k in 1:length(test_chain_ids)
            ...
            flush(stdout)
            start_time = time()
            results[k] = RunSimulation(num_vehicles, am_capacity, wc_capacity, k_max, vv, budget, depot_id, policy_type, test_chain_id, mcts_iterations, mcts_depth, mcts_num_chains, mcts_runtime, test)
...
### MAIN ###
test_chain_start_id = parse(Int64, ARGS[1])
test_chain_end_id = parse(Int64, ARGS[2])
test_chain_ids = [x for x in test_chain_start_id:test_chain_end_id]
depot_id = 2616

test = false

dirpath_base = "evaluation_iccps_mtd"

if !isdir(dirpath_base)
    mkdir(dirpath_base)
end
...
mcts_iterations = 1
mcts_depth = 1
mcts_num_chains = num_process
parallel = false

mcts_runtime = 0.1
file_path = "$(dirpath_base)/$(num_vehicles)_$(test_chain_start_id)_$(test_chain_end_id)_$(am_capacity)_$(wc_capacity)_$(k_max)_$(vv)_$(budget)_$(policy_type)_$(mcts_iterations)_$(mcts_depth)_$(mcts_num_chains)_$(mcts_runtime).csv"
results = FullSim(num_vehicles, am_capacity, wc_capacity, k_max, vv, budget, depot_id, policy_type, parallel, test, file_path, test_chain_ids, mcts_iterations, mcts_depth, mcts_num_chains, mcts_runtime)
